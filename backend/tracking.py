...
def analyze_player_movement(track_positions: list) -> dict:
    """
    Analyze movement patterns to distinguish players from spectators.

    Players characteristics:
    - High movement variance (running around court)
    - Fast movement speed
    - Position changes across court width

    Spectators characteristics:
    - Low movement variance (sitting/standing still)
    - Near-zero movement speed
    - Fixed position

    Args:
        track_positions: List of (x, y) center positions across frames

    Returns:
        dict with movement metrics: variance, speed, is_likely_player
    """
    if len(track_positions) < 5:
        return {'variance': 0, 'avg_speed': 0, 'is_likely_player': False}

    positions = np.array(track_positions)

    # Calculate position variance (how much they move around)
    variance_x = np.var(positions[:, 0])
    variance_y = np.var(positions[:, 1])
    total_variance = variance_x + variance_y

    # Calculate average speed (distance between consecutive frames)
    speeds = []
    ...
    # - Variance > 0.001 (move around the court)
    # - Average speed > 0.002 (actively moving)
    # - Max speed > 0.01 (sprinting for shots)
    is_likely_player = (
        total_variance > 0.001 or  # Significant position variance
        avg_speed > 0.002 or        # Active movement
        max_speed > 0.015           # Occasional fast movements (lunges, sprints)
    )
    ...
    return {
        ...
        'avg_speed': avg_speed,
        'max_speed': max_speed,
        'is_likely_player': is_likely_player
    }


def detect_match_format(track_summaries: list, court_region: tuple) -> str:
    """
    Detect if the match is singles or doubles based on player positions.

    Singles: 1 player per side (2 total on court)
    Doubles: 2 players per side (4 total on court)

    Args:
        track_summaries: List of TrackSummary objects
        court_region: (x, y, w, h) normalized court region

    Returns:
        'singles' or 'doubles'
    """
    if len(track_summaries) <= 2:
        return 'singles'

    # Count players per side
    near_side_count = sum(1 for s in track_summaries if s.side == 'near')
    far_side_count = sum(1 for s in track_summaries if s.side == 'far')

    # If either side has 2 players, it's doubles
    if near_side_count >= 2 or far_side_count >= 2:
        return 'doubles'
...
class PlayerTracker:
    ...
    def __init__(self, fps: int = 30, model_path: str = "yolov8n.pt"):
        ...
        # NOTE: static_image_mode=False is faster but requires consistent detection
        # Using model_complexity=2 for better accuracy on cropped regions
        self.pose = None
        if MEDIAPIPE_AVAILABLE:
            self.pose = mp.solutions.pose.Pose(
                static_image_mode=False,  # Better for video (uses tracking)
                model_complexity=2,       # Higher accuracy (0, 1, or 2)
        ...
        while cap.isOpened():
            ...
            if tracked.tracker_id is not None and len(tracked.tracker_id) > 0:
                ...
                for candidate in court_candidates:
                    ...
                    # Add LARGER padding for pose estimation (MediaPipe needs context)
                    pad = 0.25  # 25% padding on each side for better pose detection
                    x1, y1, x2, y2 = bbox_xyxy
                    bw, bh = x2 - x1, y2 - y1
                    ...
                    # Ensure minimum crop size (MediaPipe needs at least 256x256 for good results)
                    crop_w = x2_padded - x1_padded
                    crop_h = y2_padded - y1_padded
                    min_size = 200
                    if crop_w < min_size or crop_h < min_size:
                        # Expand to minimum size
                        cx, cy = (x1_padded + x2_padded) / 2, (y1_padded + y2_padded) / 2
                        half_size = max(min_size, crop_w, crop_h) / 2
                        x1_padded = max(0, cx - half_size)
                        y1_padded = max(0, cy - half_size)
                        x2_padded = min(w, cx + half_size)
                        y2_padded = min(h, cy + half_size)
                    ...
                    if self.pose is not None and cropped.size > 0 and cropped.shape[0] > 50 and cropped.shape[1] > 50:
    ...
    def get_track_summaries(self, tracking_results: list, filter_by_movement: bool = True) -> list:
        """
        Generate summary statistics for each unique track.

        Includes movement analysis to filter out spectators:
        - Players move around the court (high variance)
        - Spectators stay in one place (low variance)

        Args:
            tracking_results: List of FrameTracking from process_video
            filter_by_movement: If True, only return tracks with player-like movement

        Returns:
            List of TrackSummary objects (filtered to likely players)
        """
        track_data = {}
            ...
            for track in frame.tracks:
                ...
                if tid not in track_data:
                    track_data[tid] = {
                        ...
                        'positions': []  # For movement analysis
                    }
        ...
        for tid, data in track_data.items():
            ...
            # Analyze movement to distinguish players from spectators
            movement = analyze_player_movement(data['positions'])
            ...
            # Only include tracks with player-like movement
            if filter_by_movement:
                if movement['is_likely_player']:
                    player_candidates.append((summary, movement['variance']))
                    print(f"[Tracking] Track {tid}: PLAYER (variance={movement['variance']:.4f}, "
                          f"speed={movement['avg_speed']:.4f}, frames={data['frame_count']})")
                else:
                    print(f"[Tracking] Track {tid}: SPECTATOR (variance={movement['variance']:.4f}, "
                          f"speed={movement['avg_speed']:.4f}) - FILTERED OUT")
            else:
                summaries.append(summary)

        if filter_by_movement:
            # Sort by movement variance (most active = most likely player)
            player_candidates.sort(key=lambda x: -x[1])
            summaries = [s for s, _ in player_candidates]

        # DEDUPLICATION: Remove tracks that overlap significantly with higher-ranked tracks
        # (ByteTrack can assign different IDs to the same person when they reappear)
        deduplicated = []
        for summary in summaries:
            is_duplicate = False
            for existing in deduplicated:
                # Check if same side and similar frame range (likely same person)
                if summary.side == existing.side:
                    # Calculate frame overlap
                    overlap_start = max(summary.first_frame, existing.first_frame)
                    overlap_end = min(summary.last_frame, existing.last_frame)
                    overlap_frames = max(0, overlap_end - overlap_start)
                    min_duration = min(
                        summary.last_frame - summary.first_frame,
                        existing.last_frame - existing.first_frame
                    )
                    if min_duration > 0 and overlap_frames / min_duration > 0.5:
                        # More than 50% overlap = likely duplicate
                        print(f"[Tracking] Track {summary.track_id} is duplicate of {existing.track_id} (same side, overlapping frames)")
                        is_duplicate = True
                        break
            if not is_duplicate:
        ...
        # Detect match format based on player positions
        match_format = detect_match_format(summaries, self.court_region)
        max_players = 2 if match_format == 'singles' else 4

        print(f"[Tracking] Detected match format: {match_format.upper()} (max {max_players} players)")

        # Limit to appropriate number of players
        summaries = summaries[:max_players]

        # Sort by frame count for consistent ordering
